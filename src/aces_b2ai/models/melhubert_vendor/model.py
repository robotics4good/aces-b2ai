...
Vendored into aces_b2ai — only change: bare imports converted to relative.
  from module import ...      →  from .module import ...
  from fairseq_code import ... →  from .fairseq_code import ...
"""
import numpy as np
import torch
...
class MelHuBERTConfig:
    ...
    def __init__(self, config: dict):
        ...
        # Criterion (not used in fine-tuning / no_pred mode)
...
class MelHuBERTModel(nn.Module):
        ...
        if self.model_config.learnable_mask_emb:
            if not self.model_config.mask_before_proj:
                self.mask_emb = nn.Parameter(
                    torch.FloatTensor(model_config.encoder_embed_dim).uniform_()
                )
            else:
                self.mask_emb = nn.Parameter(
                    torch.FloatTensor(model_config.feat_emb_dim).uniform_()
                )
        else:
            self.mask_emb = 0
    ...
    def forward(
        ...
        Forward function
        Input:
            feat     (FloatTensor): B x T x D
            pad_mask (FloatTensor): B x T  (1 = valid, 0 = padding)
        """
        ...
        if not self.model_config.skip_masked:
            ...
            logit_m = self.final_proj(hidden[masked_indices])
        ...
        if not self.model_config.skip_nomask:
            ...
            logit_u = self.final_proj(hidden[nomask_indices])
